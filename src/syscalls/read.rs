...
pub fn serialize_req(state: &State, _mountpoint: Arc<Path>, fd: u16, len: usize) -> Result<()> {
  ...
  let fb_read = req::Read::create(&mut fbb, &req::ReadArgs {
    fd: Some(fb_fd),
    len: len.try_into()?
  });
...
pub fn deserialize_res<'a>(_state: &State, _mountpoint: Arc<Path>, _fd: u16, data: &'a Vec<u8>) -> Result<(&'a [u8], u64), Errno> {
